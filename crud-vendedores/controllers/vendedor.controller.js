...
class VendedorController {
  static async listar(req, res) {
    const { busqueda, tipo, page = 1, limit = 10 } = req.query;
    ...
    try {
      ...
      if (busqueda && tipo) {
        vendedores = await VendedorModel.buscarPor(busqueda, tipo, currentPage, itemsPerPage);
        total = await VendedorModel.contarBusqueda(busqueda, tipo);
      } else {
        vendedores = await VendedorModel.listarTodos(currentPage, itemsPerPage);
        total = await VendedorModel.contarTotal();
      }
